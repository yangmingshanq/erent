		...
		<table class="table table-striped table-bordered table-hover1  table-condensed">
			...
			<c:forEach items="${os}" var="o">
				<tr>
					...
					<td>
						<button oid=${o.id} class="orderPageCheckOrderItems btn btn-primary btn-xs">详情</button>

						<c:if test="${o.status=='waitDelivery'}">
							<a href="admin_order_delivery?id=${o.id}">
								<button class="btn btn-primary btn-xs">已取车</button>
							</a>
						</c:if>
					</td>
				</tr>
								...
								<c:forEach items="${o.orderItems}" var="oi">
									<tr>
										...
										<td align="right">
											...
											<span class="text-muted">${oi.number}天</span>
										</td>
										<td align="right">

											<span class="text-muted">日租价：￥${oi.product.promotePrice}</span>
										</td>
